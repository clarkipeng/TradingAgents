...
- R3 constants are intentionally fixed: title field weight `2.0`, body field weight `1.0`, ticker-family boost `1.25`, and exponential recency decay with a 30-day half-life using `published_at` (falling back to `available_at`). The eligible chunk index is cached by `(corpus_hash, as_of)` and its state digest is recorded as `index_state_hash`; ties are ordered by `evidence_id/doc_key`.
